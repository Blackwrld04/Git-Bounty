...
export async function runShow(identifier) {
    ...
    try {
        ...
        if (record) {
            ...
            console.log(boxen(`${chalk.bold.hex('#14F195')('📌 GitHub Issue #' + issue.number + ': ' + issue.title)}\n\n` +
                `• Author: ${chalk.cyan(issue.author)}\n` +
                `• URL:    ${chalk.gray(issue.url)}\n` +
                `• Labels: ${chalk.magenta(issue.labels.join(', ') || 'None')}\n\n` +
                `${chalk.bold('Description:')}\n${issue.body || 'No description provided.'}\n\n` +
                `To fund as a Gibwork bounty: ${chalk.cyan(`git bounty post ${issue.number} --reward <amount>`)}`, { padding: 1, borderStyle: 'round', borderColor: 'cyan' }));
            return;
        }
