...
export async function runSubmit(opts = {}) {
    ...
    console.log(boxen(`${chalk.bold.hex('#14F195')('⚡ Packaging Gibwork Bounty Submission')}\n` +
        `• Task ID: ${chalk.cyan(taskId)}\n` +
        (issueNumber ? `• Linked Issue: #${issueNumber}\n` : '') +
        `• Network: ${chalk.white.bold(config.network.toUpperCase())}`, { padding: 1, margin: { top: 1, bottom: 1 }, borderStyle: 'round', borderColor: 'green' }));
    ...
    try {
        ...
        console.log(boxen(`${chalk.bold.hex('#14F195')('🎉 Submission Recorded on Gibwork!')}\n\n` +
            `• Task ID: ${chalk.white(taskId)}\n` +
            `• Intent ID: ${chalk.cyan(intent.intentId || 'Confirmed')}\n` +
            `• Status: ${chalk.green.bold(intent.status || 'Fulfilled')}\n\n` +
            `The repository maintainer will review your submission and merge your PR to release the Solana escrow.`, { padding: 1, margin: { top: 1, bottom: 1 }, borderStyle: 'round', borderColor: 'green' }));
    }
    catch (error) {
