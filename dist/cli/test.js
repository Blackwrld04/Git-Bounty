...
export async function runTest(opts = {}) {
    ...
    if (result.passed) {
        ...
        console.log(boxen(`${chalk.green.bold('✔ VERIFICATION PASSED')}\n\n` +
            `Your code changes satisfy the automated test requirements.\n` +
            `Run ${chalk.cyan.bold('git bounty submit')} to submit your proof of work.`, { padding: 1, margin: { top: 1, bottom: 1 }, borderStyle: 'round', borderColor: 'green' }));
    }
    else {
        ...
        console.log(boxen(`${chalk.red.bold('✖ VERIFICATION FAILED')}\n\n` +
            `Please fix failing tests before submitting to Gibwork.`, { padding: 1, margin: { top: 1, bottom: 1 }, borderStyle: 'round', borderColor: 'red' }));
        process.exit(1);
    }
}
