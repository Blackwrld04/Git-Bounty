...
export class GitHubService {
     ...
     */
    async getIssue(issueNumber, repo) {
        const targetRepo = repo || this.repoSlug;
        const repoFlag = targetRepo ? `-R ${targetRepo}` : '';
        if (await this.hasGhCli()) {
            try {
                const { stdout } = await execAsync(`gh issue view ${issueNumber} ${repoFlag} --json number,title,body,url,labels,author`);
        ...
        throw new Error(`Cannot fetch GitHub issue #${issueNumber}. Please install and authenticate GitHub CLI ('gh auth login') or set GITHUB_TOKEN.`);
    }
    /**
     * Adds a comment to an issue or pull request
     */
    async addComment(issueOrPrNumber, comment, repo) {
        const targetRepo = repo || this.repoSlug;
        const repoFlag = targetRepo ? `-R ${targetRepo}` : '';
        if (await this.hasGhCli()) {
            try {
     ...
     */
    async addLabel(issueNumber, labelName, repo) {
        const targetRepo = repo || this.repoSlug;
        const repoFlag = targetRepo ? `-R ${targetRepo}` : '';
        if (await this.hasGhCli()) {
            try {
     ...
     */
    async getPullRequest(prNumber, repo) {
        const targetRepo = repo || this.repoSlug;
        const repoFlag = targetRepo ? `-R ${targetRepo}` : '';
        if (await this.hasGhCli()) {
            const { stdout } = await execAsync(`gh pr view ${prNumber} ${repoFlag} --json number,title,body,url,state,merged,headRefName,baseRefName`);
            const data = JSON.parse(stdout);
            return {
                number: data.number,
                title: data.title,
     ...
     */
    async mergePullRequest(prNumber, repo) {
        const targetRepo = repo || this.repoSlug;
        const repoFlag = targetRepo ? `-R ${targetRepo}` : '';
        if (await this.hasGhCli()) {
