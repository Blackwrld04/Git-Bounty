...
export class TestRunner {
  ...
  constructor(command: string = 'npm test', cwd: string = process.cwd()) {
   ...
   */
  async runTests(): Promise<VerificationResult> {
  ...
  async generateProofOfWork(
    ...
    issueNumber?: number,
    prUrl?: string
  ): Promise<{ bundle: ProofOfWorkBundle; markdown: string }> {
    ...
    const bundle: ProofOfWorkBundle = {
      taskId,
      issueNumber,
      commitHash,
      branchName,
