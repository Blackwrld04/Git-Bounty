...
export async function runSettle(prNumberStr: string, opts: SettleOptions = {}) {
  ...
  try {
    ...
    console.log(
      boxen(
        `${chalk.bold.hex('#14F195')('⚡ Solana Escrow Payout Settlement')}\n\n` +
        ...
        `• Action: ${opts.noMerge ? chalk.yellow('Release Escrow Only') : chalk.green('Merge PR & Release Escrow')}`,
    ...
    console.log(
      boxen(
        `${chalk.bold.hex('#14F195')('🏆 Bounty Settlement Complete!')}\n\n` +
        `The contributor has received their reward directly to their Solana wallet.`,
        { padding: 1, margin: { top: 1, bottom: 1 }, borderStyle: 'round', borderColor: 'green' }
      )
    );
  } catch (error: any) {
