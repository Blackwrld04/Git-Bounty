...
export async function runWallet(opts: WalletOptions = {}) {
  ...
  try {
    ...
    if (!rawKey) {
      ...
      console.log(
        boxen(
          `${chalk.bold.yellow('Wallet Setup Instructions')}\n\n` +
          ...
          `   ${chalk.cyan('solana-keygen new -o ~/.config/solana/id.json')}`,
          { padding: 1, margin: { top: 1, bottom: 1 }, borderStyle: 'round', borderColor: 'yellow' }
        )
      );
      return;
    }
    ...
    console.log(
      boxen(
        `${chalk.bold.hex('#14F195')('⚡ Solana Wallet Diagnostic')}\n\n` +
        ...
        `• Environment: ${chalk.cyan(config.network.toUpperCase())}`,
