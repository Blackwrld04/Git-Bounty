...
export async function runReview(identifier: string) {
  ...
  try {
    ...
    console.log(
      boxen(
        `${chalk.bold.hex('#14F195')('📋 Gibwork Submissions Review')}\n` +
        `• Task ID: ${chalk.cyan(taskId)}\n` +
        `• Total Submissions: ${chalk.white.bold(submissions.length)}`,
